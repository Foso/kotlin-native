...
private fun processCLib(args: Array<String>, additionalArgs: Map<String, Any> = mapOf()): Array<String>? {
    ...
    val allLibraryDependencies = when (flavor) {
        KotlinPlatform.NATIVE -> resolveDependencies(cinteropArguments, tool.target, mode)
        else -> listOf()
    }
...
private fun resolveDependencies(
        cinteropArguments: CInteropArguments, target: KonanTarget, mode: GenerationMode
): List<KotlinLibrary> {
    ...
    return resolver.resolveWithDependencies(
            ...
            noStdLib = mode == GenerationMode.SOURCE_CODE,
